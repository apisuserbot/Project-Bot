...
def handle(update):
	...
	try:
		...
		if len(queue["free"]) > 1 and not uid in queue["occupied"]:
			...
			if partner != uid:
				keyboard = ReplyKeyboardMarkup(keyboard=[
					["👋🏻"],['Next ▶️', '❌ Exit'],[dict(text='Bagikan Nomor', request_contact=True)]
				],resize_keyboard=True, one_time_keyboard=True)
