...
token = "1626343681:AAHaWPTq2zkd4ZdXcCD7rDwAj99N7UVnwOE"
	...
	try:
		...
		if text == "/start":
			if not uid in queue["occupied"]:
				keyboard = ReplyKeyboardMarkup(keyboard=[['Cari Pasangan 👥', 'Info Profile 📌'], ['Total Pengguna👤'],['Join Grup']], resize_keyboard=True)
				bot.sendMessage(uid, "Selamat Bergabung Di\nBot AnonymousMyBoo🙊\nSemoga Dapat teman atau jodoh\n\nJangan Lupa Grup @caritemanh", reply_markup=keyboard)
		...
		if text.split()[0] == "/brd":
			...
			try:
				for uid in users:
					bot.sendMessage(int(uid), text,parse_mode= 'Markdown', disable_web_page_preview=True)
			except:
				raise
			...
			if update["from"]["last_name"] != None:
				...
				date = time.strftime("%D %H:%M:%S", time.localtime(int(update["date"]))).split()
				text = "Nama : " + str(name) +"\n"
				text += "ID Kamu : " + str(_id) +"\n"
				text += "Username : @" + str(username) + "\n"
				text += "Tipe Chat : " + str(tipe) +"\n"
				text += "Tanggal : " + str(date[0]) +"\n"
				text += "Waktu⏱ : " + str(date[1]) + " WIB" "\n"
				bot.sendMessage(uid, text)
			...
			keyboard = ReplyKeyboardMarkup(keyboard=[['Cari Pasangan 👥', '🔙 Main Menu']], resize_keyboard=True)
		...
		if len(queue["free"]) > 1 and not uid in queue["occupied"]:
			...
			if partner != uid:
				...
				bot.sendMessage(uid, 'Pasangan kamu telah ditemukan, selamat halu wkwk😜', reply_markup=keyboard)
				bot.sendMessage(partner, 'Pasangan kamu telah ditemukann, selamat halu wkwk😜', reply_markup=keyboard)
